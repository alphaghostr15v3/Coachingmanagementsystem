...
<div class="mb-4 animate__animated animate__fadeIn">
    ...
    <h2 class="fw-bold">Modify User Account</h2>
</div>
    ...
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm animate__animated animate__fadeInUp">
            <div class="card-body p-4 p-md-5">
                <form action="{{ route('admin.system-users.update', $system_user) }}" method="POST">
                    @csrf
                    @method('PUT')
                    
                    <div class="mb-4">
                        <label for="coaching_id" class="form-label fw-bold small text-uppercase text-secondary">Associated Institute <span class="text-danger">*</span></label>
                        <select name="coaching_id" id="coaching_id" class="form-select form-select-lg border-0 bg-light rounded-4 @error('coaching_id') is-invalid @enderror" required>
                            @foreach($coachings as $coaching)
                                <option value="{{ $coaching->id }}" {{ old('coaching_id', $system_user->coaching_id) == $coaching->id ? 'selected' : '' }}>{{ $coaching->coaching_name }}</option>
                            @endforeach
                        </select>
                        @error('coaching_id')
                    ...
                    <div class="mb-4">
                        ...
                        <div class="d-flex gap-4">
                            <div class="form-check custom-option w-50">
                                <input class="form-check-input d-none" type="radio" name="role" id="role_student" value="student" {{ old('role', $system_user->role) == 'student' ? 'checked' : '' }} onchange="toggleFields()">
                            ...
                            <div class="form-check custom-option w-50">
                                <input class="form-check-input d-none" type="radio" name="role" id="role_teacher" value="teacher" {{ old('role', $system_user->role) == 'teacher' ? 'checked' : '' }} onchange="toggleFields()">
                    ...
                    <div class="row g-4 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" id="name" class="form-control form-control-lg border-0 bg-light rounded-4 @error('name') is-invalid @enderror" value="{{ old('name', $system_user->name) }}" required>
                            @error('name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="text" name="phone" id="phone" class="form-control form-control-lg border-0 bg-light rounded-4 @error('phone') is-invalid @enderror" value="{{ old('phone', $extraData->phone ?? '') }}">
                            @error('phone')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="email" name="email" id="email" class="form-control form-control-lg border-0 bg-light rounded-4 @error('email') is-invalid @enderror" value="{{ old('email', $system_user->email) }}" required>
                        @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div id="student_fields" class="mb-4">
                        ...
                        <textarea name="address" id="address" rows="2" class="form-control form-control-lg border-0 bg-light rounded-4 @error('address') is-invalid @enderror" placeholder="Enter complete address">{{ old('address', $extraData->address ?? '') }}</textarea>
                        @error('address')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div id="teacher_fields" class="mb-4" style="display: none;">
                        ...
                        <input type="text" name="subject" id="subject" class="form-control form-control-lg border-0 bg-light rounded-4 @error('subject') is-invalid @enderror" value="{{ old('subject', $extraData->subject ?? '') }}" placeholder="e.g. Mathematics, Physics">
                        @error('subject')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row g-4 mb-5">
                        <div class="col-md-6">
                            <label for="password" class="form-label fw-bold small text-uppercase text-secondary">Reset Password <span class="text-muted lowercase small">(Leave blank to keep current)</span></label>
                    ...
                    <div class="d-flex justify-content-end gap-3 pt-3 border-top">
                        <a href="{{ route('admin.system-users.index') }}" class="btn btn-light px-4 py-3 rounded-4 fw-bold border-0 shadow-sm text-secondary">Discard Changes</a>
                        <button type="submit" class="btn btn-gradient px-5 py-3 rounded-4 fw-bold shadow-lg">
                            <i class="fas fa-save me-2"></i> Update User Record
                        </button>
                    </div>
                </form>
            </div>
