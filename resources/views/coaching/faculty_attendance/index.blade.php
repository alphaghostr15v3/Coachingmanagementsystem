...
<div class="d-flex justify-content-between align-items-center mb-4 animate__animated animate__fadeIn">
    <div>
        <h2 class="fw-bold mb-1 text-dark">Faculty Attendance Report</h2>
        <p class="text-muted small">Comprehensive tracking and reporting for faculty attendance history.</p>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('coaching.faculty-attendance.index') }}" class="btn btn-light shadow-sm px-4 rounded-4 border">
            <i class="fas fa-sync-alt me-2"></i> Reset
        </a>
        <a href="{{ route('coaching.faculty-attendance.create', ['date' => $date]) }}" class="btn btn-primary shadow-sm px-4 rounded-4">
...
<div class="card border-0 shadow-sm mb-4 animate__animated animate__fadeInUp" style="animation-delay: 0.1s;">
    <div class="card-body p-4">
        <form method="GET" action="{{ route('coaching.faculty-attendance.index') }}" class="row g-3 align-items-end">
            <div class="col-md-4">
                <label class="form-label small fw-bold text-uppercase text-muted">Select Faculty</label>
                <select name="faculty_id" class="form-select border-0 bg-light rounded-3 py-2">
            ...
            <div class="col-md-3">
                ...
                <select name="month" class="form-select border-0 bg-light rounded-3 py-2">
                    ...
                    @for($i = 1; $i <= 12; $i++)
                        <option value="{{ $i }}" {{ (request('month') ?? (request('faculty_id') ? date('m') : '')) == $i ? 'selected' : '' }}>{{ date('F', mktime(0, 0, 0, $i, 1)) }}</option>
                    @endfor
                </select>
            </div>
            <div class="col-md-3">
            ...
            @if(!request('faculty_id') && !request('month'))
