...
<div class="mb-4 animate__animated animate__fadeIn">
    ...
    <h2 class="fw-bold">Modify Announcement</h2>
</div>
    ...
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm animate__animated animate__fadeInUp">
            <div class="card-body p-4 p-md-5">
                <form action="{{ route('coaching.notices.update', $notice) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" id="title" class="form-control form-control-lg border-0 bg-light rounded-4 @error('title') is-invalid @enderror" value="{{ old('title', $notice->title) }}" required>
                        @error('title')
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="description" id="description" rows="5" class="form-control border-0 bg-light rounded-4 @error('description') is-invalid @enderror" required>{{ old('description', $notice->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="d-flex justify-content-end gap-3 pt-3 border-top">
                        <a href="{{ route('coaching.notices.index') }}" class="btn btn-light px-4 py-2 rounded-4 fw-bold border">Back</a>
                        <button type="submit" class="btn btn-primary px-5 py-2 rounded-4 fw-bold shadow">
                            <i class="fas fa-sync-alt me-2"></i> Update Announcement
                        </button>
                    </div>
                </form>
            </div>
