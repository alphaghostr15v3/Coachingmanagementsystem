...
<div class="mb-4 animate__animated animate__fadeIn">
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <h2 class="fw-bold m-0">Teacher Attendance History</h2>
            <p class="text-muted small mb-0">Manage and track daily attendance records for all teachers.</p>
        </div>
        <div class="d-flex gap-2">
            <form action="{{ route('coaching.teacher-attendance.index') }}" method="GET" class="d-flex gap-2">
                <input type="date" name="date" class="form-control border-0 shadow-sm rounded-4" value="{{ $date }}" onchange="this.form.submit()">
            </form>
            <a href="{{ route('coaching.teacher-attendance.create', ['date' => $date]) }}" class="btn btn-primary rounded-4 fw-bold shadow-sm px-4">
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="bg-light">
                    <tr>
                        <th class="ps-4 py-3 text-secondary small text-uppercase">Teacher</th>
