...
<head>
    ...
    <style>
        ...
        @keyframes pulse-indigo {
            0% { box-shadow: 0 10px 15px -3px rgba(79, 70, 229, 0.3); }
            50% { box-shadow: 0 10px 25px -3px rgba(79, 70, 229, 0.5); }
            100% { box-shadow: 0 10px 15px -3px rgba(79, 70, 229, 0.3); }
        }
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="nav-menu">
            ...
            <a href="{{ route('teacher.notices') }}" class="{{ request()->routeIs('teacher.notices') ? 'active' : '' }}">
        ...
        <div class="topbar">
            ...
            <div class="dropdown">
                <button class="btn d-flex align-items-center dropdown-toggle border-0" type="button" data-bs-toggle="dropdown">
                    <span class="fw-semibold me-2 d-none d-md-inline">{{ auth()->user()->name }}</span>
                    @php
                        $sidebarTeacher = \App\Models\Teacher::where('email', auth()->user()->email)->first();
                    @endphp
                    @if($sidebarTeacher && $sidebarTeacher->profile_image)
                        <img src="{{ asset($sidebarTeacher->profile_image) }}" alt="{{ $sidebarTeacher->name }}" class="rounded-circle shadow-sm" style="width: 35px; height: 35px; object-fit: cover;">
                    @else
                ...
                <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-4 mt-2">
                    <li>
                        <a class="dropdown-item py-2" href="{{ route('teacher.profile') }}">
